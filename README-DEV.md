...
- builds **Linux, Android, Windows and macOS** binaries — including a Windows
  installer (`.exe`, Inno Setup) and Linux **`.deb` / `.rpm`** packages — and
  attaches them to the release (iOS is handled separately via App Store Connect),
...
`scripts/build-linux.sh` builds the Linux bundle and packages it three ways into
`dist/linux/`: a portable **tar.gz** (with a per-user `install.sh`), a **`.deb`**
(via `dpkg-deb`) and an **`.rpm`** (via [`fpm`](https://github.com/jordansissel/fpm),
which the CI installs with `gem install fpm` plus `rpm` for the rpmbuild
backend). All install the bundle under `/opt/jellymusic` with a
`/usr/bin/jellymusic` symlink, a `.desktop` entry and a hicolor icon. Run it
locally with `VERSION=x.y.z scripts/build-linux.sh` (`.rpm` is skipped when fpm
isn't installed). The `.deb` depends on the usual desktop libs (gtk, glib,
libsecret, libmpv, libayatana-appindicator); the `.rpm` gets its shared-library
requires auto-generated by rpm.
