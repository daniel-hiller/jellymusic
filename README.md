...
- **Library** — Albums, Artists, Songs, Playlists, Genres. Sort (name / artist /
  year / date added / play count / random), a favourites filter, in-tab search,
  infinite scroll and a server-side A–Z rail on desktop.
- **Home** — shelves mirroring what Jellyfin tracks: continue listening, recently
  played (albums *and* tracks), recently added, most played, favourites, a random
  pick. Each hides itself when empty.
- **Player** — responsive Now Playing: cover + Queue/Lyrics on desktop, a single
  ...
  cover colour. The queue is also its own screen, reachable from the mini player.
...
- **Playback** — gapless (libmpv prefetch on desktop) with an optional fade,
  background playback + OS media controls, and scrobbling back to Jellyfin.
- **Themes** — 12 palettes (6 dark / 6 light), switchable at runtime.
- **Accounts** — multiple servers/users, password or **Quick Connect** login.
- **More** — Instant Mix radio, playlist CRUD, on-device response caching,
  German/English localization, and an in-app update check.
